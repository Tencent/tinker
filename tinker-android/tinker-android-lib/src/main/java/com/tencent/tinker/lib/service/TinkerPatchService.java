...
public class TinkerPatchService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent == null) {
            TinkerLog.d(TAG, "TinkerPatchService received a null intent, ignoring.");
            return;
        }
        String path = getPatchPathExtra(intent);
        if (path == null) {
            TinkerLog.d(TAG, "TinkerPatchService can't get the path extra, ignoring.");
            return;
        }
        ...
        try {
            if (isUpgradePatch) {
                ...
                result = upgradePatchProcessor.tryPatch(context, path);
                ...
                result = repairPatchProcessor.tryPatch(context, path);
            }
        ...
        PatchResult patchResult = new PatchResult(isUpgradePatch, result, path, cost, e);
