...
public abstract class DecodedInstruction {
    ...
    public static void decodeAll(short[] encodedInstructions, DecodedEventCallback cb) {
        ShortArrayCodeInput in = new ShortArrayCodeInput(encodedInstructions);
        try {
            while (in.hasMore()) {
                if (cb != null) {
                    cb.onDecoded(DecodedInstruction.decode(in));
                }
            }
        } catch (EOFException ex) {
            throw new DexException(ex);
    ...
    public interface DecodedEventCallback {
        void onDecoded(DecodedInstruction decodedInstruction);
    }
}
