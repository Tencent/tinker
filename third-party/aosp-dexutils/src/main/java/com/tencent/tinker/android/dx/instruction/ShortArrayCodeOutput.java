...
public final class ShortArrayCodeOutput extends CodeCursor {
    ...
    public void write(short u0, short u1) {
        ensureCapacity(SizeOf.USHORT * 2);
        write(u0);
        write(u1);
    }
    ...
    public void write(short u0, short u1, short u2) {
        ensureCapacity(SizeOf.USHORT * 3);
        write(u0);
        write(u1);
        write(u2);
    }
    ...
    public void write(short u0, short u1, short u2, short u3) {
        ensureCapacity(SizeOf.USHORT * 4);
        write(u0);
        write(u1);
        write(u2);
        write(u3);
    ...
    public void write(short u0, short u1, short u2, short u3, short u4) {
        ensureCapacity(SizeOf.USHORT * 5);
        write(u0);
        write(u1);
        write(u2);
        write(u3);
    ...
    public void write(byte[] data) {
        ensureCapacity(SizeOf.UBYTE * data.length);
        int value = 0;
        boolean even = true;
        for (byte b : data) {
            if (even) {
    ...
    private void ensureCapacity(int bytesToWritten) {
        int currPos = cursor();
        if ((array.length - currPos) * SizeOf.USHORT < bytesToWritten) {
