 ...
 * Changes class name from ByteArray to ByteArrayInput.
 * MyDataInputStream was removed since we needn't use it in
 * DexDiff library.
 */
public final class ByteArrayInput {
     ...
     *              the slice (exclusive)
     */
    public ByteArrayInput(byte[] bytes, int start, int end) {
        if (bytes == null) {
     ...
     */
    public ByteArrayInput slice(int start, int end) {
        checkOffsets(start, end);
        return new ByteArrayInput(bytes, start + this.start, end + this.start);
    }
     ...
     */
    public static class MyDataInputStream extends DataInputStream implements GetCursor {
     ...
     */
    public class MyInputStream extends InputStream implements GetCursor {
        /**
         * 0..size; the cursor
         */
        private int cursor;
